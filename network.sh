...
function createOrgs() {
  ...
  if [ -d "${HOME_DIR}/organizations/peerOrganizations" ]; then
    rm -Rf ${HOME_DIR}/organizations/peerOrganizations && rm -Rf ${HOME_DIR}/organizations/ordererOrganizations
  fi
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=${HOME_DIR}/config/crypto-config-org1.yaml --output=${HOME_DIR}/organizations
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    set -x
    cryptogen generate --config=${HOME_DIR}/config/crypto-config-org2.yaml --output=${HOME_DIR}/organizations
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
    ...
    set -x
    cryptogen generate --config=${HOME_DIR}/config/crypto-config-orderer.yaml --output=${HOME_DIR}/organizations
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    . ${HOME_DIR}/organizations/fabric-ca/registerEnroll.sh
  ...
  ${HOME_DIR}/organizations/ccp-generate.sh
}
...
function createConsortium() {
  ...
  set -x
  configtxgen -profile TwoOrgsOrdererGenesis -channelID system-channel -outputBlock ${HOME_DIR}/config/artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
function networkUp() {
  ...
  if [ "${EXPLORER}" == "yes" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f ${COMPOSE_FILE_EXPLORER}"
  fi
...
## Bring up the network if it is not arleady up.
  ...
  ${HOME_DIR}/scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
  if [ $? -ne 0 ]; then
...
function deployCC() {
  ...
  ${HOME_DIR}/scripts/deployCC.sh $CHANNEL_NAME $CC_SRC_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
...
COMPOSE_FILE_BASE=${HOME_DIR}/docker/docker-compose-net.yaml
# docker-compose.yaml file if you are using couchdb
COMPOSE_FILE_COUCH=${HOME_DIR}/docker/docker-compose-couch.yaml
# certificate authorities compose file
COMPOSE_FILE_CA=${HOME_DIR}/docker/docker-compose-ca.yaml
# Explorer compose file
COMPOSE_FILE_EXPLORER=${HOME_DIR}/docker/docker-compose-explorer.yaml
